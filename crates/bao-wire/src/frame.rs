...
#[cfg(test)]
mod tests {
    use bao_core::protocol::{Request, Rpc};
    ...
    async fn round_trips_typed_messages() {
        let (a, b) = duplex(4096);
        let mut writer = FrameWriter::<_, Request>::new(a);
        let mut reader = FrameReader::<_, Request>::new(b);

        writer.write(&request()).await.unwrap();

        let got = reader.read().await.unwrap().expect("one message");
        assert_eq!(got.id, 7);
        assert!(matches!(got.rpc, Rpc::List));
    }
    ...
    async fn oversized_frame_is_rejected() {
        ...
        let mut reader = FrameReader::<_, Request>::new(a);
